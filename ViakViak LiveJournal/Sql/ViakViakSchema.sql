...
exec spAddArticle 44207, N'Что в имени твоем: Двор', N'Вашкевич, вяк, слова', N'
<span viak="word">Двор

ДВР - двор дверь доверие
ДбР - добро дебри
тВР - товар тварь творить товарищ Тверь утварь отвар творог
РВт - рвота рвать
Рбт - работа робот Арбат орбита рубить
тбР - табор отбор Тибр
</span><span viak="description">
Рассмотрим приведенные ниже толкования слов, где Товар сводится к понятию "рядом" и родственности слову "тор", а Товарищ еще и сравнивается с близким по смыслу арабским "тарака" - следовать друг за другом, стучать(в смысле периодически: один стук за другим). Учтем, что "в" может, оказывается, переходить в "о". Тогда мы можем увидеть, что корень ДВР может перейти в ДОР, прочитывается наоборот как "РОД" или "РЯД". Мы можем видеть также, что переход "В - О" позволяет объяснить слова Товар и Товарищ без привлечения арабского языка:
</span><span viak="word">
ДР - дыра дар
РД - ряд рядом орда
Рт - рот
тР - тор тора
Рж - ружье, (ок)руж(ность) рыжий ряженый рожа
жР - жир жар
</span><span viak="summary">
Представляется, что слово Двор лежит в смысловом контексте "рядом" и означает пространство рядом с домом.
</span><span viak="description">
дверь - выход в комнату рядом
добро - то что надо хранить рядом
...
Тверь - город неподалеку, рядом.
творить - созданное специально, должно близко по смыслу к слову "торить" при выпадении "В".
творог - сотворенный
